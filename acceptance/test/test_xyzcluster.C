...
void test_xyzcluster()
{
  ...
  PIXYZCluster xyzcluster;
  xyzcluster.radius(0.13);
  const auto clusters = xyzcluster.cluster_hits(hitptrs.begin(), hitptrs.end());

  for (const auto &cluster : clusters) {
    std::cout << "Reference point: " << *cluster.first << ", ";
