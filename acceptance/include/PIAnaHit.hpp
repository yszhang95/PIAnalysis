...
{
public:
  PIAnaHit(): PIAnaHit(0, 0, -1E9, -1, -1E9, -1E9, -1,
                       -1, -1, 0, -1, false) {}
  PIAnaHit(double const x, double const y, double const z,
           ...
           bool const valid):
             x_(x), y_(y), z_(z), edep_(edep), post_t_(post_t), dt_(dt),
...
//  ~PIAnaHit();
  ...
  static int find_layer(double const);
  static int find_strip(double const);
